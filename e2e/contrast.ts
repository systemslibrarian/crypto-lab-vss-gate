 ...
 *    `color-mix()`. This page is built out of both: `body` carries a linear
 *    gradient plus two radial tints, every `.exhibit` and `.card` is a
 *    translucent `hsl(... / 0.84)` over it, and the hero aside is a
 *    `color-mix()` wash. axe files those nodes under `incomplete` rather than
 *    measuring them, which is where nothing was looking — and it is exactly
 *    that stacking that this helper caught: `--accent` at 36% lightness cleared
 *    4.5:1 on near-white but only reached 4.32:1 on the hero aside, where the
 *    page's cyan radial shows through.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The footer was an inline
 *    `opacity: .7` around already-muted text and links, which is how it managed
 *    to fail in BOTH themes at once; the hero subtitle renders at `.85` and
 *    disabled inputs at `.5`. Note this lab collapses its animations to 0.01ms
 *    under reduced motion, which preserves end states.
 *
 ...
 *    boxes are the `.table-wrap` scrollers around each result table, which hold
 *    long hex curve points, so at phone width much of every table is outside
 *    its scroller's client box. Content scrolled past that box is not dimmed or
 *    partly drawn — it is absent from the frame, and
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks four verification tables of hex curve points,
     * the share-curve SVG, four lab steps of prose and four teaching
     * disclosures, and every one of those nodes re-walks the same handful of
     ...
     * that is the `.table-wrap` scrollers around each result table.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has four of them — the teaching disclosures
      // and the crypto-parameters panel, all shut on arrival.
      // `checkVisibility()` catches them, and the gate opens each by clicking
      // its <summary> — the route a reader has — rather than setting `.open`
      // from script, which is what the previous gate did to all of them at
      // once.
